...
class ThermocyclerView {
  ...
  public:
    ...
    void updateGUI() {
      ...
      screen->print(toString(buff, this->model->coldBath->temperature, 2), LEFT, base);
      deselect();

      if (this->model->isSelected(&this->model->hotBath->temperature)) {
        selected();
      }
      screen->print(toString(buff, this->model->hotBath->temperature, 2), RIGHT, base);
      deselect();
      ...
      screen->print(toString(buff, this->model->coldBath->time, 3), LEFT, base);
      deselect();

      if (this->model->isSelected(&this->model->hotBath->time)) {
        selected();
      }
      screen->print(toString(buff, this->model->hotBath->time, 3), RIGHT, base);
      deselect();
      base = base + segmentOffset;
      ...
      screen->print(toString(buff, this->model->cycles, 5), CENTER, base - 50);
      deselect();
      ...
      drawButton("START", 180, base, this->model->isSelected(&this->model->start));
      deselect();
